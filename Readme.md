...
CC[^CC]は、そのような障壁を取り除き、heis対戦用AIを容易に作成するための基盤プラットフォームとして作成されました。
...
2. CCをビルドする。
   ...
   2. Visual Studioが開くので、ビルド構成は"Release"、プラットフォーム構成は"x86"でビルドする[^VS]。
...
詳しくは、doc/説明書/AI実装ガイド.pdfをご覧ください。
...
詳しくは、HeisClient_CC/Tool/HeisPseudoServer/Readme.pdfをご覧ください。
...
[^CC]: 本ソフト(Compreheisive Client)の略称。
[^VS]: Visual Studioの操作方法が不明な場合は、doc/説明書/VS使い方ガイド.pdfをご参照ください。
